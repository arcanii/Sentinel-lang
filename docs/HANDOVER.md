...
> 3. **scg self-host mirror** — bring the M2.x sealed/stdio/arg/generic-channel builtins (AND ADR 0070's `Type::Fn`/`apply`/direct-call type-check+codegen lowering) into scg parity. The FnId BASES are already mirrored (corpus green); what's missing = the per-builtin LOWERING in `selfhost/types/cg*.sentinel` + (for ADR 0070) a `Type::Fn` cg mirror (the arithmetic-id scheme in `sentinel-types` needs no interner mirror, just the cg lowering arms — the D3-revisit unification adds no new scg-mirror surface beyond what M-cont already left, since it's types-stage-only and its new fixtures are types-corpus-excluded). Medium effort, project-identity value, rush-dangerous (do carefully).
...
> Also still pending (older, lower-priority): **M2.4c-2** (generic word-scalar `Type::SealedChannel` — cosmetic, low value, do BEFORE the scg mirror if at all); the scg mirror of M2.3b. See the two-items-deferred note below (superseded by item 3 above, which now also covers ADR 0070) for the M2.4 tails' ROI/risk.
