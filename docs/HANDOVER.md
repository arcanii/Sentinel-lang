...
### ▶ RESUME HERE (2026-07-02 — M1.4a IN PROGRESS: slice 1 (runtime cell) DONE + committed → NEXT: slice 2a, the coordinated FnId-base shift)

> **▶ NEXT STEP — M1.4a slice 2a: the coordinated FnId-base shift 38→40.** Slice 1 (the `SentinelShared` refcounted runtime cell + `sentinel_shared_new`/`_clone`/`_get`/`_release` + unit tests + abi count 38→42 + abi-v1.md rows) is DONE and committed (`d73322c`) — pure runtime, symbols dormant until codegen calls them. The plan is at `C:\Users\bryan\.claude\plans\misty-nibbling-shore.md`; ADR 0071 is the design of record; the two Explore reports (SealedChannel recipe + clone/drop insertion points) are in this session's task outputs.
>
> **SEQUENCING (refined after slice 1 — important):** the plan's original "all-Rust then all-selfhost" order is WRONG for the FnId shift — adding `shared_new`/`shared_get` shifts the user-fn base 38→40, and FnIds appear in every corpus fixture's dump, so a Rust-only shift leaves the self-host differential RED. **Land the FnId shift + type plumbing in BOTH compilers together per slice:** slice 2a = register the two builtins as reserved slots in `snc` (`sentinel-resolve` FnId consts + base arithmetic, `sentinel-types` sigs + golden-dump assertions) AND `selfhost` (`resolve.sentinel` `builtin_id`, the hardcoded `fid-38`→`fid-40` arithmetic in `cg.sentinel`/`cg_effects.sentinel`/etc., `mir` names) + the 4 driver golden dumps — no fixture calls them yet so the differential stays green; slice 2b = `Type::Shared(SharedId)` + `SharedData` interner table (the `Channel<T>` template) + `resolve_type_expr` + `cgo_ty`→ptr + `is_copy_type`=true + lowering, both compilers + a create/get fixture (`needs_drop` still false); slice 3 = the clone/drop accounting (flip `needs_drop`=true, the `emit_drop_for_binding`/`cg_emit_drop` release arm, the rc++ clone sites at let-init/call-arg/spawn-capture gated on named-`Var` && `Shared`, the early-return transfer exemption) + leak-checked fixtures, both compilers. Runtime underflow-assert + free-exactly-once fixtures are the safety net for the accounting (the one novel/high-risk piece).
>
> HANDOFF STATE: four-check green through slice 1; working tree clean (slice 1 committed); the 18 known pre-existing Windows failures unchanged. Read ADR 0071 D2/D3/D8 + the plan's "refcount accounting" + "return/transfer" notes before slice 3 (the early-`return`-of-a-`Shared` double-release gotcha).
>
