...
### ▶ RESUME HERE (2026-07-01 — scg self-host mirror slice DONE: stdin_recv/stdout_send/arg_count/arg → NEXT: a fresh-session DECISION, see the menu below)

> **▶ NEXT DECISION (fresh session) — the scg self-host mirror gap is now PARTIALLY CLOSED: `stdin_recv`/`stdout_send`/`arg_count`/`arg` (FnId 33-36) are fully lowered in scg, verified byte-identical via a new `tests/pass` differential fixture. HANDOFF STATE: four-check green (`cargo build` + `cargo test --workspace --no-fail-fast` + doctests + `clippy -D warnings`, all under the vcvars MSVC shell), all 9 selfhost differential stages byte-identical (both bootstrap fixed points hold), working tree CLEAN. Known pre-existing Windows-only failures (NOT regressions, exactly 19 across 5 targets, unchanged): `pass_c5d4_file_io` (`/tmp`), `tests/modules.rs` `separate_*_linkonce_*` (MSVC link / `nm`), `tests/llvm.rs`'s `llvm_behaviour_matches_inkwell_*` (hardcodes the Unix runtime), the `export --shared` tests (DLL export unsupported on Windows), the POSIX-FFI + socket `examples/` (macOS-only). Pick a direction:**
> 1. **Finish the scg self-host mirror** — 5 gaps remain, precisely scoped by this session's own research: `sealed_channel`/`sealed_process` (FnId 31/32 — needs a NEW `SealedChannel` type interner kind, ~11-site ripple like `Type::Process` originally needed, but the bridge builtins' own lowering is a trivial identity-ptr passthrough); `Type::Fn`/`apply`/the direct-call unification (FnId 37 — needs scg's FIRST indirect-call codegen shape, genuinely novel, not a copy of an existing pattern); generalizing `Channel<T>`/`process_send`/`process_recv` beyond `i64` (scg's existing i64-only paths are correct and differential-clean today — this is "incomplete relative to snc," not "broken," so lowest urgency of the 3). Do the SealedChannel bridge next (closest in shape to what was just done); save `Type::Fn`/`apply` for last (the hardest, most novel piece). Rush-dangerous, do carefully — same caution as this session.
> 2. **Full capturing closures** — ADR 0024 D10's original deferral, still not done: environment capture + lifetime tracking interacting with the lexical borrow checker. The biggest, most foundational rock still on the table; ADR-first, a real language-feature design.
...
> Also still pending (older, lower-priority): **M2.4c-2** (generic word-scalar `Type::SealedChannel` — cosmetic, low value, do BEFORE the sealed_channel scg mirror if at all); the scg mirror of M2.3b's generic elements (folded into item 1's "Channel<T> beyond i64" above).
