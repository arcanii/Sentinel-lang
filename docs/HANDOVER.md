...
### ▶ RESUME HERE (2026-07-02 — ADR 0071 (`Shared<T>` + `Mutex<T>`) PINNED; M1.4-0 gate DONE → NEXT: M1.4a, the `Shared<T>` refcounted handle)

> **▶ NEXT STEP — M1.4a: build the `Shared<T>` refcounted handle** (ADR 0071 D2/D3/D8), the first `Copy`-for-the-checker YET drop-emitting handle. This is the hard part of M1.4 (the Copy-vs-drop tension) and the prerequisite for `Mutex<T>` (M1.4b) and secret (M1.4c). Read ADR 0071 in full first — every D-point is PINNED with the concrete touchpoints from the 5-agent research pass. HANDOFF STATE: ADR 0071 PROPOSED+PINNED + committed; M1.4-0 (the D5 gate) done — 2 examples + the analysis, honest finding "channels mostly suffice, proceeding on `Shared<T>`'s standalone value"; four-check green (build + `cargo test --workspace --no-fail-fast` + doctests + `clippy -D warnings`, vcvars MSVC shell); no compiler change yet so the 9 selfhost differential stages are untouched; working tree has the M1.4-0 examples/docs (to be committed). Known pre-existing Windows-only failures (NOT regressions, exactly 18 across 4 targets): `tests/modules.rs` `separate_*_linkonce_*` (MSVC link / `nm`), `tests/llvm.rs`'s `llvm_behaviour_matches_inkwell_*` (hardcodes the Unix runtime), the `export` tests (`--shared` unsupported on Windows; 3 need a `cc` on PATH), the POSIX-FFI + socket `examples/` (macOS-only).
>
> **M1.4a scope (from ADR 0071):** a `Type::Shared` interner kind (17, generic over element `T`) + the runtime `struct SentinelShared { rc: AtomicUsize, value }` (Arc's inner behind the C-ABI: `sentinel_shared_new`/`_clone`/`_release`/`_get`) + the codegen clone/drop accounting (rc++ at duplication-into-new-owner from a named binding, rc-- at scope-exit drop, `#++==#--` invariant, leak-checked) + `is_copy_type`=true but a NEW `needs_drop`=true (the first handle to decouple them) with a dedicated `emit_drop_for_binding` arm emitting `sentinel_shared_release` + abi-v1 symbol-count bump + the FULL selfhost mirror (byte-identical). Then M1.4b `Mutex<T> = Shared<SentinelMutex<T>>` + fallible `lock()` + guard-no-escape + D5a; then M1.4c secret.
>
> **Deferred/other directions still open** (if the maintainer pivots off M1.4): full capturing closures (ADR 0024 D10, the biggest remaining language rock); the D3-revisit direct-call syntax (`op(x)`) unmirrored in scg's `dump_te_call`; generalizing `Channel<T>`/`process_send`/`process_recv` beyond `i64` in scg; **M2.4c-2** (generic word-scalar `Type::SealedChannel`); BACKLOG §11.6/§11.7 + the ADR-0067 self-host-modularization tails + the standing macOS verification.
