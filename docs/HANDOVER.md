...
### ▶ RESUME HERE (2026-07-02 — scg self-host mirror: `Type::Fn`/`apply` codegen DONE too, the full 6-gap effort now closed with zero scope cuts → NEXT: a fresh-session DECISION, see the menu below)

> **▶ NEXT DECISION (fresh session) — the scg self-host mirror is now COMPLETE, including codegen (see the DONE entry below: the codegen-stage oracle, `llvm_dump.rs`, is ported and scg's first indirect-call codegen shape is byte-identical). The only remaining `Type::Fn`/`apply` gap is the D3-revisit direct-call syntax, `op(x)` instead of `apply(op,x)`, staying unmirrored in scg's `dump_te_call` — a distinct, separately-scoped follow-up, not part of the 6 originally-tracked gaps. HANDOFF STATE: four-check green (`cargo build` + `cargo test --workspace --no-fail-fast` + doctests + `clippy -D warnings`, all under the vcvars MSVC shell), all 9 selfhost differential stages byte-identical (both bootstrap fixed points hold), working tree CLEAN. Known pre-existing Windows-only failures (NOT regressions, exactly 18 across 4 targets, unchanged — down from 19/5, since `pass_c5d4_file_io` was FIXED this session, see the DONE entry below): `tests/modules.rs` `separate_*_linkonce_*` (MSVC link / `nm`), `tests/llvm.rs`'s `llvm_behaviour_matches_inkwell_*` (hardcodes the Unix runtime), the `export` tests (`--shared` unsupported on Windows; 3 others need a `cc` on PATH, absent here), the POSIX-FFI + socket `examples/` (macOS-only). Pick a direction:**
> 1. **Full capturing closures** — ADR 0024 D10's original deferral, still not done: environment capture + lifetime tracking interacting with the lexical borrow checker. The biggest, most foundational rock still on the table; ADR-first, a real language-feature design.
...
> 3. **Switch areas** — BACKLOG §11.6 (`return value;` as a fn terminator) / §11.7 (cross-module `pub class`), the ADR-0067 self-host-modularization tails, or the standing macOS verification.
> Also still pending (older, lower-priority, all "incomplete relative to snc" — not broken): generalizing `Channel<T>`/`process_send`/`process_recv` beyond `i64` in scg (snc's `i64`-only paths there are correct and differential-clean today); **M2.4c-2** (generic word-scalar `Type::SealedChannel` — cosmetic, low value); the D3-revisit direct-call syntax (`op(x)`) unmirrored in scg's `dump_te_call` (needs the same "vars win over fns" disambiguation the Rust resolver already has).
