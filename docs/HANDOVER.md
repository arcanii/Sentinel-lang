...
### ▶ RESUME HERE (2026-07-02 — M1.4a IN PROGRESS: slices 1 (runtime cell) + 2a (FnId-base shift) DONE + committed → NEXT: slice 2b, `Type::Shared` type + lowering)

> **▶ NEXT STEP — M1.4a slice 2b: `Type::Shared(SharedId)` + lowering, both compilers, + a create/get fixture (`needs_drop` still FALSE — leaks like `Channel`; the clone/drop accounting is slice 3).** Slice 1 (the `SentinelShared` refcounted runtime cell + `sentinel_shared_new`/`_clone`/`_get`/`_release` + unit tests + abi count 38→42 + abi-v1.md rows) is committed (`d73322c`); slice 2a (the coordinated FnId-base shift 38→40 — the two builtins registered as reserved slots in BOTH compilers + the 4 driver golden dumps) is committed (`e1e1c9f`). The plan is at `C:\Users\bryan\.claude\plans\misty-nibbling-shore.md`; ADR 0071 is the design of record. `shared_new` = FnId 38, `shared_get` = FnId 39, user-fn base = 40 in both `snc` and `scg`; `check_call`/`dump_te_call` have NO dispatch arm for them yet, so calling them does not compile — slice 2b adds that.
>
> **Slice 2b concretely (follow the `Channel<T>` GENERIC template, NOT SealedChannel's plain-unit shape):** Rust `sentinel-types` — a `Type::Shared(SharedId)` variant + a `SharedData { elem }` interner table on `TypedProgram` + `intern_shared`; the mechanical arms (`resolve_type_expr` "Shared" name → validate/intern the word-scalar element, `type_display`/`Debug`, `no_nullable`/`no_array`/`no_vec`, `substitute`/`contains_type_param` passthrough); the `check_call` special-cases for `SHARED_NEW_FN_ID` (context-typed element, encode into the i64 slot like `channel_new`/`send`) and `SHARED_GET_FN_ID` (decode the element from the `Shared<T>` arg, the `recv`/`process_recv` pattern) — REPLACING the slice-2a placeholder sig blocks. `sentinel-codegen` — `cgo_ty(Shared)`→ptr, `is_copy_type`=true, the `lower_*` (new→`sentinel_shared_new`, get→`sentinel_shared_get`) + the `module.add_function` runtime-fn decls; `sentinel-borrow-check` `is_copy_type`=true. The `llvm_dump.rs` oracle — `llvm_ty(Shared)`→ptr + the `lower_call` arms for FnId 38/39 (so the differential can reach them). selfhost mirror — `interner.sentinel` kind 17 `mk_shared` + `render_type`, `cg.sentinel` `cg_is_shared` + `cgo_ty`, `cg_effects.sentinel` `cg_emit_call` arms, `builtin_ret` entries. At this slice `field_type_needs_drop(Shared)` stays **FALSE** (leaks like Channel). Add `tests/pass/` fixture(s) that create + `shared_get` a `Shared<i64>` (and a couple word-scalar elements), exit-validated. **The full "add a handle type" recipe:** `git show b789a08` (SealedChannel scg mirror) + its snc-side predecessor + the `Channel<T>` add for the generic-element handling; ADR 0071 D9 summarizes the touchpoints.
>
> **SEQUENCING (from the plan, refined after slice 1):** land the FnId shift + type plumbing in BOTH compilers together per slice. 2a (FnId-base shift, DONE) → 2b (`Type::Shared` + lowering + create/get fixture, `needs_drop` FALSE) → slice 3 = the clone/drop accounting (flip `needs_drop`=true, the `emit_drop_for_binding`/`cg_emit_drop` release arm, the rc++ clone sites at let-init/call-arg/spawn-capture gated on named-`Var` && `Shared`, the early-return transfer exemption) + leak-checked fixtures, both compilers. Runtime underflow-assert + free-exactly-once fixtures are the safety net for the accounting (the one novel/high-risk piece).
>
> HANDOFF STATE: four-check green through slice 2a (build clean; `cargo test --workspace --no-fail-fast` = exactly the 18 known pre-existing Windows failures, zero new; all 9 self-host differential stages byte-identical, both bootstrap fixed points hold; doctests + clippy clean); working tree clean (2a committed `e1e1c9f`). Read ADR 0071 D2/D3/D8 + the plan's "refcount accounting" + "return/transfer" notes before slice 3 (the early-`return`-of-a-`Shared` double-release gotcha).
>
