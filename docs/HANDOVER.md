...
>      `Type::Ref(rid)` to `refs[rid].inner`); both Rust emitters then computed `self` with
>      `lower_lvalue_ptr`, which returns the receiver's own STORAGE. For an owned `k: K`
>      that slot holds the object and the address is right; for `c: &K` it holds a POINTER,
>      so the callee got one indirection too many. `fn read(c: &K) -> i64 { c.peek() }`
>      answered a frame pointer instead of 42, and `fn poke(c: &mut K) { c.bump() }` left
>      `k.v` unchanged — `self.v = 99` stored somewhere else. No diagnostic on either, on
>      ordinary source. The IR showed it plainly: `%v0 = alloca ptr` / `store ptr %arg0,
>      ptr %v0` / `call @K__peek(ptr %v0)`.
>
>      Root cause is a predicate mismatch of the shape this register has recorded before:
>      the type layer dereferenced the receiver and codegen did not. Both emitters now key
>      the load on the same `Type::Ref` the type layer matched, behind a
>      `lower_receiver_ptr` seam. `self` is untouched — it is bound to the object pointer
>      under the CLASS type, not a reference.
>
...
>      **⚠ NEEDS A MAINTAINER DECISION — options tabulated in
>      [`open-decisions.md`](open-decisions.md).**
...
>
>      **D29 — DONE (`fbccc2b`).** `scg`'s CHANNEL arms (fid 22/23) dispatched the
...
>
>      **D57 — the constant-time compare fold `1 - (((0 - acc) >> 63) & 1)` is a test for
>      `acc > 0`, not for `acc != 0`, and it is the ACCEPT/REJECT decision of every
>      verifier that uses it.** For any NEGATIVE `acc`, `0 - acc` is positive, its sign bit
>      is clear, and the fold returns ACCEPT. Constructed: acc = -1, -128, -255 all yield 1.
>      **Not currently exploitable anywhere**, and the reason is an invariant none of the
>      sites states: `u8` is unsigned (`docs/abi-v1.md`), so an XOR of two byte-derived
>      `i64`s lands in [0,255] and `acc` cannot go negative. That is true and it is not a
>      thing to rest a tag check on.
>      Fixed in `std/security/aead.sentinel`'s new open with `acc | (0 - acc)`, which sets
>      the sign bit for every non-zero value either way. **FOUR OTHER SITES still carry the
>      bare form** — `std/net/ssh_cipher.sentinel`, `std/security/sealed_kex.sentinel`,
>      `std/security/ed25519.sentinel`, `std/security/ed448.sentinel` — plus two more
>      instances of the bare `((0 - acc) >> 63) & 1` half. They are structurally safe for
>      the same unsigned reason; ⚠ in ssh_cipher and sealed_kex the compared bytes are
...
>
>      **D54 — DONE (this slice).** `--emit-header` rendered a `&mut [u8]` export
>      parameter as `const uint8_t*`. `emit_c_header` pushes `"const uint8_t*"` unconditionally
...
>
>      **D56 — DONE (this slice).** The generated C header did not name the Windows system
>      libraries a foreign host must link. The set lives in `WINDOWS_NATIVE_LIBS` and the
>      EXECUTABLE link path has always used it, but a host linking the `--lib` ARCHIVE had
>      no way to learn it — not in the header, not in `--lib` output, not in ADR 0059. A
>      downstream integrator found it from **33 unresolved externals** and concluded
>      "always link all six", which is a workaround rather than an answer. `--emit-header`
>      now emits the list as a comment plus a `#pragma comment(lib, …)` block guarded by
>      `_MSC_VER` and `SENTINEL_NO_AUTOLINK` (ADR 0059 A12).
>      ⚠ **THE REQUIREMENT IS NOT INSPECTABLE, which is why emitting beats documenting.**
>      Which libraries are needed depends on which Rust std paths that particular Sentinel
>      program reaches — a value-only library that never allocates links fine with none of
>      them — so a host cannot derive it by reading its own code, only by failing.
>      Verified end-to-end: a C host naming NO system libs links against the archive
>      following the header alone and runs; the same host with `/DSENTINEL_NO_AUTOLINK`
>      fails with exactly the reported symbol class, so the pragmas are load-bearing and
>      the opt-out is a real opt-out. Header text only — non-oracle-moving, no `scg`
>      mirror.
>
>      **D55 — DONE (this slice).** `docs/STATE.md` listed two SHIPPED capabilities as
>      "still deferred". It
