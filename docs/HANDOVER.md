...
For pasting into a fresh chat to bootstrap context:
    ...
    **RESUME HERE → D.6 follow-ups (multi-file + ALL cross-module item kinds +
    generics + effect-check all work):**
    ...
    link-time dedup is the (2/N) piece). (b) **span-accurate multi-source
    diagnostics** — the merged path reports by message; spans point into
    per-module sources, so a diagnostic can name the wrong file. (c) true
    per-unit resolve → (3/N) caching. ADR 0037 "Implementation notes" has the
    design. **NOTE: the language gate for the self-host port (D5) is
    effectively cleared** — multi-file + all item kinds + generics + sound
    effect-check all work under Path A; (a)/(b)/(c) are quality/architecture,
    not blockers.
