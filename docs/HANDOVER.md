...
For pasting into a fresh chat to bootstrap context:
    ...
    RESUME AT: **Phase D.2 (3/N) — strings + `u8`: the type layer.** (1/N
    lexer + 2/N AST+parser shipped; 1296 tests.)
    ...
    **D.2 — strings + a byte type (ADR 0033 PROPOSED).** The load-bearing
    ...
    `u8`↔`i64` conversions. **4-sub-phase split (ADR 0033 D7):**
      ...
      - **(3/N) types** — `Type::U8` + **the cascade** across every exhaustive
        `Type` match + `ArrayElem::U8` (the ADR 0032 (3/N) discipline — land
        the arms together, build, then semantics); char→`u8`, string→`[u8]`,
        `str_eq`/conversion builtins typed; resolve→typed mirrors (the
        `CharStringLitNotYet` reject is replaced by real lowering). ← RESUME
      - (4/N) codegen + runtime — `i8`; string-literal global + heap copy;
        `sentinel_str_eq`; conversions; `abi-v1` `u8` entry + tests +
        `c5d2_strings` phase-go (verified leak-free via `leaks --atExit`);
        ADR flip.
