...
### ▶ RESUME HERE (2026-07-02 — M1.4a IN PROGRESS: slices 1 (runtime cell) + 2a (FnId-base shift) + 2b (`Type::Shared` + lowering) DONE + committed → NEXT: slice 3, the clone/drop refcount accounting — the risky slice)

> **▶ NEXT STEP — M1.4a slice 3: the clone/drop refcount accounting (the one genuinely novel/high-risk slice).** Slice 1 (the `SentinelShared` runtime cell + the 4 C-ABI symbols + underflow debug-assert) is committed (`d73322c`); slice 2a (the coordinated FnId-base shift 38→40) `e1e1c9f`; slice 2b (`Type::Shared(SharedId)` + `shared_new`/`shared_get` type-check + lowering in both compilers + fixture `c71_shared.sentinel`, `needs_drop` still FALSE — the handle is Copy and LEAKS like `Channel`) `d3eafe6`. The plan is at `C:\Users\bryan\.claude\plans\misty-nibbling-shore.md`; ADR 0071 D2/D3/D8 is the design of record. So today: `Shared<i64>` create + `shared_get` works end-to-end (exit 42), Copy-duplicable, but the cell is never freed (rc never decremented). Slice 3 makes it a real refcounted handle.
>
> **Slice 3 concretely — flip `needs_drop`→true + emit the accounting, both compilers (READ ADR 0071 D2/D3 + the plan's "refcount accounting" + "return/transfer" notes FIRST):**
> 1. **needs_drop=true.** `field_type_needs_drop_inner(Shared)` → true (Rust, `crates/sentinel-codegen/src/lib.rs` — the arm I set to `false` at 2b with a "slice 3 flips this" comment); the selfhost `cg_needs_drop` equivalent → true. This makes a `Shared` binding recorded for scope-exit drop.
> 2. **rc-- (release) at scope-exit drop.** The drop-walk arm (Rust `emit_drop_for_binding`/the per-type drop match — the `Type::Shared(_) => {}` no-op I left at 2b with a "slice 3" comment; selfhost `cg_emit_drop`) emits `call @sentinel_shared_release(ptr)` — the `Array` free-arm template. Declare `sentinel_shared_release`/`_clone` in codegen + the `llvm_dump.rs` oracle (`RuntimeSyms` field + `merge()` + `emit_declares()` + its return OR-chain — ALL FOUR sites, the 2b gotcha) + the selfhost `cg_used_*` (field + declare group + `cg_anydecl` + tyctx init).
> 3. **rc++ (clone) at each duplication of a NAMED `Shared` binding into a new owner** — exactly 3 sites: a `let` initializer, a by-value call arg, a spawn capture. Emit `call @sentinel_shared_clone(ptr)`. Gate: Rust `expr.kind == TypedExprKind::Var(_)` && `ty` is `Shared`; selfhost `(*c).mvbv >= 0` after `dump_texpr` && `cg_is_shared`. An **rvalue** source (a direct `shared_new(...)` result — a non-`Var`) TRANSFERS its unit → NO clone.
> 4. **The subtle one — early `return a;` of a `Shared`.** `Shared` is Copy so the borrow checker records no move → `emit_return_drops` would release `a` AND the caller releases the returned handle → double-release/underflow. Fix: exempt an early-returned named `Shared` from the return drain (the analogue of the existing `tail_returned` exemption; tail-return `{ …; a }` already works via `tail_returned`). Read `emit_return_drops` precisely.
> - **Invariant: #clone + #new == #release.** Safety net: the slice-1 runtime underflow debug-assert + leak-checked `tests/pass` fixtures asserting free-exactly-once (mirror `c24_moved_array_no_double_free`'s style). **De-risk by sequencing the fixtures:** let-dup + arg-pass + spawn + tail-return FIRST (prove the core green), THEN early-return + its exemption as its own fixture.
> - Oracle-moving → full self-host mirror, both bootstrap fixed points byte-identical.
>
> **Then M1.4b (`Mutex<T>`) + M1.4c (secret `T`)** are separate later plans (see ADR 0071 D1/D4/D6).
>
> HANDOFF STATE: four-check green through slice 2b (build clean; `cargo test --workspace --no-fail-fast` = exactly the 18 known pre-existing Windows failures, zero new; `pass` 142→143 with `c71_shared`; all 9 self-host differential stages byte-identical, both bootstrap fixed points hold; doctests + clippy clean); working tree clean (2b committed `d3eafe6`). Two easily-missed oracle touchpoints the 2b differential caught and that slice 3 must also honor when adding `_clone`/`_release` declares: `RuntimeSyms::merge` (per-fn used-flags OR'd into the module set) and `emit_declares`'s return OR-chain (drives the trailing blank line).
>
> **Deferred/other directions** (if the maintainer pivots off M1.4): full capturing closures (ADR 0024 D10); the D3-revisit direct-call syntax (`op(x)`) unmirrored in scg; generalizing `Channel<T>`/`process_send`/`process_recv` beyond `i64` in scg; **M2.4c-2**; BACKLOG §11.6/§11.7 + ADR-0067 tails + macOS verification.
