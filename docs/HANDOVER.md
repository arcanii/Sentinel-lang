...
For pasting into a fresh chat to bootstrap context:
    ...
    RESUME AT: **Phase D.2 (2/N) — strings + `u8`: AST + parser.** (1/N
    lexer shipped.)
    ...
    **D.2 — strings + a byte type (ADR 0033 PROPOSED).** The load-bearing
    ...
    `u8`↔`i64` conversions. **4-sub-phase split (ADR 0033 D7):**
      ...
      - **(2/N) AST + parser** — `ExprKind::CharLit(u8)` + `StringLit(Vec<u8>)`
        ...
        exactly one byte); `u8` in `TypeExpr` (resolves to `Type::U8` at
        3/N). Resolve pass-through; types reject (NotYet) until 3/N. ← RESUME
