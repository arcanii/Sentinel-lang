...
For pasting into a fresh chat to bootstrap context:
    ...
    RESUME AT: **Phase D.2 (4/N) — strings + `u8`: codegen + runtime.** (1/N
    lexer + 2/N AST+parser + 3/N type layer shipped; 1309 tests. char/string
    type-check; codegen rejects them until 4/N.)
    ...
    **D.2 — strings + a byte type (ADR 0033 PROPOSED).** The load-bearing
    ...
    `u8`↔`i64` conversions. **4-sub-phase split (ADR 0033 D7):**
      ...
      - **(4/N) codegen + runtime** — `i8` char const; string-literal global
        `[N x i8]` + heap copy; `sentinel_str_eq`; `zext`/`trunc` conversions;
        `abi-v1` `u8` entry + tests +
