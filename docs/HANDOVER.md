...
### ▶ RESUME HERE (2026-07-01 — scg self-host mirror: sealed_channel/sealed_process DONE too, 5 of 6 gaps closed → NEXT: a fresh-session DECISION, see the menu below)

> **▶ NEXT DECISION (fresh session) — the scg self-host mirror gap is now DOWN TO ONE ITEM: `stdin_recv`/`stdout_send`/`arg_count`/`arg` (FnId 33-36) AND `sealed_channel`/`sealed_process` (FnId 31/32, incl. a NEW `SealedChannel` type interner kind) are fully lowered in scg, both verified byte-identical via new `tests/pass` differential fixtures. HANDOFF STATE: four-check green (`cargo build` + `cargo test --workspace --no-fail-fast` + doctests + `clippy -D warnings`, all under the vcvars MSVC shell), all 9 selfhost differential stages byte-identical (both bootstrap fixed points hold), working tree CLEAN. Known pre-existing Windows-only failures (NOT regressions, exactly 19 across 5 targets, unchanged): `pass_c5d4_file_io` (`/tmp`), `tests/modules.rs` `separate_*_linkonce_*` (MSVC link / `nm`), `tests/llvm.rs`'s `llvm_behaviour_matches_inkwell_*` (hardcodes the Unix runtime), the `export --shared` tests (DLL export unsupported on Windows), the POSIX-FFI + socket `examples/` (macOS-only). Pick a direction:**
> 1. **Finish the scg self-host mirror — the last gap** — `Type::Fn`/`apply`/the direct-call unification (FnId 37): needs scg's FIRST indirect-call codegen shape (an LLVM `call` through a function-pointer VALUE, not a named `@symbol` — genuinely novel, not a copy of an existing pattern) PLUS a `Type::Fn` interner representation (mirror the Rust arithmetic `FnValueSigId` scheme, or a simpler cg-only special-case — decide which when you get there). This is the hardest of the 6 original gaps, saved for last on purpose. Rush-dangerous, do carefully. (Generalizing `Channel<T>`/`process_send`/`process_recv` beyond `i64` stays separately low-urgency: scg's `i64`-only paths are correct and differential-clean today — "incomplete relative to snc," not "broken.")
...
> Also still pending (older, lower-priority): **M2.4c-2** (generic word-scalar `Type::SealedChannel` — cosmetic, low value; the scg mirror is already the fixed-`secret i64` shape, so this would touch both compilers again if ever done).
