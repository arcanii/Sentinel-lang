...
- 🟢 **Phase C — Bootstrap compiler** (production Rust implementation, targets LLVM) — **C0–C4 complete; C5 (productionization toward 1.0) nearly complete:**
...
- **Not production-ready, and not yet 1.0.** The full language (types, generics, borrow check + RAII, secret + effect typing, the handler runtime, classes/traits/delegation, structured concurrency) compiles and runs, and the 1.0 acceptance program's close bar is met — but the formal 1.0 declaration, plus tooling polish (LSP), are still pending.
