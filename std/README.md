...
| ----------- | -------------------- | ---------------------------------------- |
| `security`  | `ct`, `siphash`, `chacha20`, `poly1305`, `aead`, `sha256`, `sha512`, `hmac`, `aes`, `x25519` | ✅ ct scalars + `ct_memcmp` over `[secret u8]` + `ct_vec_eq` over `Vec<secret u8>` + `ct_rotl64`/`ct_rotl32`/`ct_rotr32`/`ct_rotr64`; `siphash24` keyed MAC; `chacha20_block` + `chacha20_xor` stream cipher; `poly1305` one-time MAC; `chacha20poly1305_encrypt` AEAD (full RFC 8439 §2.8.2 vector); `sha256` constant-time SHA-256 + `sha512` constant-time SHA-512 over a `secret` message; `hmac_sha256` over a `secret` key; `aes128_encrypt_block` constant-time AES-128 (field-inversion S-box, no table); `x25519` constant-time X25519 ECDH over a `secret` scalar (RFC 7748) |
