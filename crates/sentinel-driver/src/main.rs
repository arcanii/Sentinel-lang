...
/// ADR 0059 / register D56: write the Windows system-library dependency into the
/// generated header — as `#pragma comment(lib, …)` for MSVC, and as a plain
/// comment for everyone else.
///
/// A Sentinel `--lib` archive bundles the Rust runtime staticlib, which pulls in
/// native deps the MSVC linker will not resolve on its own. The list lives in
/// [`WINDOWS_NATIVE_LIBS`] and the executable link path already uses it, but a
/// FOREIGN host linking the archive itself had no way to learn it: it was in no
/// header, no `--lib` output and no ADR. A downstream integrator discovered it
/// from **33 unresolved externals** (`__imp_WSAStartup`, `__imp_NtWriteFile`,
/// `__imp_GetUserProfileDirectoryW`, …) and concluded "always link all six",
/// which is a workaround rather than an answer.
///
/// ⚠ **THE REQUIREMENT IS NOT INSPECTABLE, WHICH IS WHY IT MUST BE EMITTED.** It
/// depends on which Rust std paths the particular Sentinel program happens to
/// pull in: a value-only library that never allocates links fine with none of
/// these. So a host cannot derive the answer by reading its own code — it can
/// only discover it by failing. Emitting the superset is the point.
///
/// Guarded on `_MSC_VER`, i.e. on the CONSUMING compiler, not on the host that
/// ran `snc` — a header generated on macOS for a Windows consumer still carries
/// it. `SENTINEL_NO_AUTOLINK` opts out for a host that manages its own link
/// line, because `#pragma comment(lib, …)` injects `/DEFAULTLIB:` directives and
/// a host with a deliberate CRT model should keep control of that.
fn emit_windows_autolink(out: &mut String) {
    out.push_str(
        "/* Windows/MSVC: this archive bundles the Sentinel runtime, which needs these\n\
         \x20  system libraries. They are auto-linked below; define SENTINEL_NO_AUTOLINK\n\
         \x20  to manage the link line yourself. Whether a given library is REQUIRED\n\
         \x20  depends on which runtime paths your Sentinel code reaches, so link the\n\
         \x20  whole set rather than trying to work it out by inspection:\n",
    );
    for lib in WINDOWS_NATIVE_LIBS {
        out.push_str(&format!("     {lib}\n"));
    }
    out.push_str(" */\n");
    out.push_str("#if defined(_MSC_VER) && !defined(SENTINEL_NO_AUTOLINK)\n");
    for lib in WINDOWS_NATIVE_LIBS {
        out.push_str(&format!("#pragma comment(lib, \"{lib}\")\n"));
    }
    out.push_str("#endif\n\n");
}
...
///
/// TWO consumers (register D56): the executable link path, and
/// [`emit_windows_autolink`], which writes the list into the generated C header
/// so a FOREIGN host linking the archive can learn it from the header rather
/// than from 33 unresolved externals. The header consumer is unconditional —
/// the guard is `_MSC_VER` in the emitted text, not the platform running `snc`
/// — so this is no longer dead on any target and carries no `allow`.
