...
/// `(fn #<id> <name> <Effect>…)` line per function, and only 5 of the 19
/// comparisons carry anything past the bare name — so on real programs this
/// sweep is chiefly a fn-table + FnId-ordering parity check. Effect ANALYSIS
/// parity is pinned by the SEEDS and the fixture corpus above (a mutant that
/// dropped the analysis entirely still fails here, on those 5). Rejections are
...
#[test]
fn sentinel_effect_checker_matches_oracle_on_real_programs() {
    let tmp =
        std::env::temp_dir().join(format!("snc_selfhost_effects_prog_{}", std::process::id()));
    let _ = std::fs::remove_dir_all(&tmp);
    std::fs::create_dir_all(&tmp).expect("create temp dir");
    let checker = build_sentinel_effect_checker(&tmp);
    let work = tmp.join("work");
    ...
    real_program_differential("effects", &checker, &work, 13, 13);
    let _ = std::fs::remove_dir_all(&tmp);
}
