...
//! occupy `FnId 0..=34`, so user fns start at `#35`; the auto-registered
...
fn resolve_dump_params_var_call() {
    ...
    // (user fns start at #35, after the 35 builtins — 0..13 + sockets 14..20 +
    // channels 21..24 + subprocess spawn/wait 25..26 + write/read 27..28 +
    // send/recv 29..30 + sealed bridge 31..32 + stdin/stdout 33..34); `main` is
    // #36. The built-in `Async` then `Subprocess` (ADR 0066 M2.1) effects are emitted last.
    assert_eq!(
        resolve_dump(
            "fns",
            "fn add(a: i64, b: i64) -> i64 { a + b }\nfn main() -> i64 { add(1, 2) }\n"
        ),
        "(fn #35 add ((param #0 a i64) (param #1 b i64)) i64 (block (binop + (var #0) (var #1))))\n\
         (fn #36 main () i64 (block (call #35 (int 1) (int 2))))\n\
         (effect #0 Async)\n\
         (effect #1 Subprocess)\n"
    );
}
...
fn resolve_dump_let_bindings() {
    ...
    assert_eq!(
        ...
        "(fn #35 main () i64 (block (let #0 i64 (int 5)) (let #1 _ (binop + (var #0) (int 1))) (var #1)))\n\
         (effect #0 Async)\n\
         (effect #1 Subprocess)\n"
    );
}
...
fn resolve_dump_builtin_call() {
    ...
    assert_eq!(
        ...
        "(fn #35 main () i64 (block (call #0 (int 42))))\n(effect #0 Async)\n(effect #1 Subprocess)\n"
    );
}
...
fn resolve_dump_struct_lit_and_field() {
    ...
    assert_eq!(
        ...
        "(struct #0 Box (field v i64))\n\
         (fn #35 main () i64 (block (let #0 _ (struct-lit #0 Box (field v (int 42)))) (field (var #0) v)))\n\
         (effect #0 Async)\n\
         (effect #1 Subprocess)\n"
    );
}
