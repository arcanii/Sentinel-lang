...
fn sentinel_borrow_checker_matches_oracle_on_real_programs() {
    ...
    real_program_differential("borrow", &checker, &work, 13, 13);
    let _ = std::fs::remove_dir_all(&tmp);
}

/// ADR 0041 A14 (register D97): the self-hosted borrow checker refuses what the oracle refuses,
/// for the refusals `scg` ports (`common::assert_refuses_what_the_oracle_refuses`).
#[test]
fn sentinel_borrow_checker_refuses_what_the_oracle_refuses() {
    let tmp = std::env::temp_dir()
        .join(format!("snc_selfhost_borrow_checker_refusals_{}", std::process::id()));
    let _ = std::fs::remove_dir_all(&tmp);
    std::fs::create_dir_all(&tmp).expect("create temp dir");
    let driver = build_sentinel_borrow_checker(&tmp);
