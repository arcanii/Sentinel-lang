...
pub const PROCESS_WAIT_FN_ID: FnId = FnId(26);
/// `process_write(p: Process, data: [u8]) -> i64` — write `data` to the child's
/// stdin then close it (M2.2); 0 ok, -1 error. `[u8]` is public → secret fence.
pub const PROCESS_WRITE_FN_ID: FnId = FnId(27);
/// `process_read(p: Process) -> [u8]` — read the child's stdout to EOF (M2.2).
pub const PROCESS_READ_FN_ID: FnId = FnId(28);
/// `process_send(p: Process, v: i64) -> i64` — frame `v` (8-byte LE) to the child's
/// stdin, keep it open; 0 ok, -1 error (M2.3 typed framed channel over the pipe).
/// `i64` is a public element → the cross-process secret fence is structural (D8).
pub const PROCESS_SEND_FN_ID: FnId = FnId(29);
...
// ADR 0066 M2.4 follow-on: own command-line argument reflection — so a spawned child
// can read its own invocation (e.g. a role flag), symmetric to `process_spawn`
// passing argv to the child. Wrapping `sentinel_arg_count`/`sentinel_arg`. Appending
// these shifts the user-fn FnId base 35 → 37.
/// `arg_count() -> i64` — the number of command-line args to THIS process (argv[0]
/// included, the C convention).
pub const ARG_COUNT_FN_ID: FnId = FnId(35);
...
pub fn resolve_module(
    ...
    next_fn_id += 1;
    // ADR 0066 M2.1: the subprocess builtins. `process_spawn(path, args)` (arity
    // 2) -> Process; `process_wait(p)` (arity 1) -> i64. Concrete signatures typed
    // in sentinel-types; codegen lowers them to `sentinel_process_*`.
    let process_spawn_sig = mk_socket_sig(next_fn_id, "process_spawn", 2);
    next_fn_id += 1;
    let process_wait_sig = mk_socket_sig(next_fn_id, "process_wait", 1);
    next_fn_id += 1;
    // ADR 0066 M2.2: the byte-pipe IPC builtins. `process_write(p, data)` (arity
    // 2) -> i64; `process_read(p)` (arity 1) -> [u8]. Concrete signatures typed
    // in sentinel-types; codegen lowers them to `sentinel_process_write/_read`.
    let process_write_sig = mk_socket_sig(next_fn_id, "process_write", 2);
    next_fn_id += 1;
    let process_read_sig = mk_socket_sig(next_fn_id, "process_read", 1);
    next_fn_id += 1;
    // ADR 0066 M2.3: the typed framed-channel-over-pipe builtins. `process_send(p,
    // v)` (arity 2) -> i64; `process_recv(p)` (arity 1) -> ?i64. Concrete signatures
    // typed in sentinel-types; codegen lowers them to `sentinel_process_send/_recv`.
    let process_send_sig = mk_socket_sig(next_fn_id, "process_send", 2);
    next_fn_id += 1;
    ...
    // (arity 0) -> i64; `arg(i)` (arity 1) -> [u8]. Codegen lowers them to
    // `sentinel_arg_count` / `sentinel_arg`.
    let arg_count_sig = mk_socket_sig(next_fn_id, "arg_count", 0);
    next_fn_id += 1;
    ...
    fn_table.insert("process_spawn".to_string(), PROCESS_SPAWN_FN_ID);
    fn_table.insert("process_wait".to_string(), PROCESS_WAIT_FN_ID);
    fn_table.insert("process_write".to_string(), PROCESS_WRITE_FN_ID);
